...
export class InventarioLoteController {
    ...
    @ApiOperation({ 
        ...
        description: 'Crea un nuevo lote de inventario para el control de costos y Kardex' 
    })
    ...
    async create(@Body() createInventarioLoteDto: CreateInventarioLoteDto): Promise<ResponseInventarioLoteDto> {
        const lote = await this.inventarioLoteService.create(createInventarioLoteDto);
    ...
    async findOne(@Param('id', ParseIntPipe) id: number): Promise<ResponseInventarioLoteDto> {
    ...
    @ApiParam({ name: 'idInventario', description: 'ID del inventario', type: 'number' })
    @ApiResponse({ 
        status: HttpStatus.OK, 
    ...
    async findByInventario(@Param('idInventario', ParseIntPipe) idInventario: number): Promise<ResponseInventarioLoteDto[]> {
        const lotes = await this.inventarioLoteService.findByInventario(idInventario);
        return lotes.map(lote => plainToClass(ResponseInventarioLoteDto, lote));
    }
    ...
    async findActiveLotes(@Query('idInventario') idInventario?: number): Promise<ResponseInventarioLoteDto[]> {
        const lotes = await this.inventarioLoteService.findActiveLotes(idInventario);
        return lotes.map(lote => plainToClass(ResponseInventarioLoteDto, lote));
    }
    ...
    @ApiOperation({ 
        ...
        description: 'Obtiene los lotes que están próximos a vencer en los días especificados' 
    })
    @ApiQuery({ 
        name: 'dias', 
    ...
    async findLotesProximosVencer(
        ...
        const lotes = await this.inventarioLoteService.findLotesProximosVencer(dias, idInventario);
        return lotes.map(lote => plainToClass(ResponseInventarioLoteDto, lote));
    }
    ...
    async findLotesVencidos(@Query('idInventario') idInventario?: number): Promise<ResponseInventarioLoteDto[]> {
        const lotes = await this.inventarioLoteService.findLotesVencidos(idInventario);
        return lotes.map(lote => plainToClass(ResponseInventarioLoteDto, lote));
    }
    ...
    @ApiParam({ name: 'numeroLote', description: 'Número de lote a buscar', type: 'string' })
    @ApiResponse({ 
        status: HttpStatus.OK, 
        description: 'Lotes encontrados',
        type: [ResponseInventarioLoteDto]
    })
    async findByNumeroLote(@Param('numeroLote') numeroLote: string): Promise<ResponseInventarioLoteDto[]> {
    ...
    @ApiOperation({ 
        ...
        description: 'Calcula el costo promedio ponderado de un inventario basado en sus lotes activos' 
    })
    @ApiParam({ name: 'idInventario', description: 'ID del inventario', type: 'number' })
    @ApiResponse({ 
        status: HttpStatus.OK, 
    ...
    async getCostoPromedioPonderado(@Param('idInventario', ParseIntPipe) idInventario: number): Promise<any> {
        const costoPromedio = await this.inventarioLoteService.getCostoPromedioPonderado(idInventario);
        return {
            idInventario,
    ...
    async update(
        ...
        const lote = await this.inventarioLoteService.update(id, updateInventarioLoteDto);
        return plainToClass(ResponseInventarioLoteDto, lote);
    }
    ...
    @ApiOperation({ 
        ...
        description: 'Consume stock de los lotes usando el método FIFO (primero en entrar, primero en salir)' 
    })
    @ApiParam({ name: 'idInventario', description: 'ID del inventario', type: 'number' })
    @ApiBody({ 
        schema: {
    ...
    async consumirStock(
        ...
        return await this.inventarioLoteService.consumirStock(idInventario, cantidad);
    }
    ...
    @ApiOperation({ 
        ...
        description: 'Elimina un lote de inventario (soft delete - marca como inactivo)' 
    })
    @ApiParam({ name: 'id', description: 'ID del lote', type: 'number' })
    @ApiResponse({ 
        status: HttpStatus.OK, 
