...
export class CategoriaService {
     ...
     */
    async create(createCategoriaDto: CreateCategoriaDto, personaId: number): Promise<ResponseCategoriaDto> {
        ...
        if (existingCategoria) {
            throw new ConflictException('Ya existe una categoría con este nombre para esta empresa');
        }
     ...
     */
    async findAll(personaId: number, includeInactive: boolean = false): Promise<ResponseCategoriaDto[]> {
    ...
    async findOne(id: number, personaId: number): Promise<ResponseCategoriaDto> {
        ...
        if (!categoria) {
            throw new NotFoundException(`Categoría con ID ${id} no encontrada para esta empresa`);
        }
     ...
     */
    async update(id: number, updateCategoriaDto: UpdateCategoriaDto, personaId: number): Promise<ResponseCategoriaDto> {
        const categoria = await this.categoriaRepository.findOne({
            where: { 
                id,
                persona: { id: personaId }
     ...
     */
    async findByName(nombre: string, personaId: number): Promise<ResponseCategoriaDto[]> {
